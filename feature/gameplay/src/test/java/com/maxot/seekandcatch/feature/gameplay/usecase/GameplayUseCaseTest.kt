...
class GameplayUseCaseTest {
    ...
    fun calculateCountOfSuitableFigures_fiveFiguresWithThreeCorrect_returnThree() {
        val figures = listOf<Figure>(
            Figure(
                FigureType.Circle, Color.Red
            ), Figure(
                FigureType.Triangle, Color.Red
            ), Figure(
                FigureType.Circle, Color.Blue
            ), Figure(
                FigureType.Square, Color.Red
            ), Figure(
                FigureType.Circle, Color.Yellow
            )
        )

        val goals = setOf<Goal<Any>>(
            Goal.Figured(
                Figure(
                    FigureType.Circle, null
                )
            )
        )
